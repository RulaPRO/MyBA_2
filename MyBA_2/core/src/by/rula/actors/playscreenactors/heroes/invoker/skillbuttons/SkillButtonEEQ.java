 ...
 */
public class SkillButtonEEQ extends SkillButton {

    private Invoker invoker;
    private Sprite icon;
    private int index;

    public SkillButtonEEQ(Invoker hero) {
        super(hero);
        invoker = hero;
        icon = new Sprite(new TextureRegion(new Texture(Gdx.files.internal("invoker_skill_map_640x64.png"))), 64 * 6, 0, 64, 64);
        index = 8;

        addListener(new InputListener() {
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
                System.out.println("Im EEQ");
                invoker.setAnimations(Invoker.EEQ);
                return true;
            }
