 ...
 */
public class SkillButtonEEE extends SkillButton {

    private Invoker invoker;
    private Sprite icon;
    private int index;

    public SkillButtonEEE(Invoker hero) {
        super(hero);
        invoker = hero;
        icon = new Sprite(new TextureRegion(new Texture(Gdx.files.internal("invoker_skill_map_640x64.png"))), 64 * 8, 0, 64, 64);
        index = 12;

        addListener(new InputListener() {
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
                System.out.println("Im EEE");
                invoker.setAnimations(Invoker.EEE);
                invoker.createSunStrike();
                return true;
            }
